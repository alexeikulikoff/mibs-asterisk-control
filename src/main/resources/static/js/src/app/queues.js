var queues = queues || {}, soundpath = null, qplayer, queueTable = null, queueAllAgentsTable = null, queueDetailTable = null;
...
queues.initAgents = function() {
	...
	$.ajax({
				...
				success : function(agents) {
					...
					$("#queues-agentid").append(
							$('<option value="-1" >' + $label.selectAll
									+ '</option>'));
					for (var i = 0; i < agents.length; i++) {
						$("#queues-agentid").append(
								$('<option value="' + agents[i].id + '" >'
										...
										columns : [
												...
												
												{
													...
													render : function(data,
														...
														return '<button type="button" class="btn btn-primary btn-xs" onclick="queues.showQueueSpell(\''
																+ page
																+ '\',\''
																+ row.id
																+ '\')">'
...
queues.setupUI = function() {
	...
	$("#queues-btn-apply").click(function() {
		...
		if (parseInt($("#queues-agentid option:selected").val()) == -1) {
			...
			var selector = "#queues-agentid option:selected";
			var agentid = $(selector).val();
			queues.showQueueSpell(1, agentid);
		}
						...
						columns : [
								...
								},
								{
									...
									render : function(data, type, row) {
										return '<button type="button" class="btn btn-primary btn-xs" onclick="queues.queueDetail(\''
												+ page
												+ '\',\''
												+ row.date
												+ '\',\''
	...
	$.ajax({
		...
		success : function(dataSet) {
			...
			if (dataSet.records == null) {
				...
				queues.createQueueDetailTable(dataSet, page, date, enterTime,
						exitTime, peer, queue);
			}
...
queues.createQueueDetailTable = function(e, page, date, enterTime, exitTime,
		peer, queue) {

	$("#inbound-queues-detail-container").empty();
	$("#inbound-queues-detail-container")
			.append(
					'<div class="col-md-12"><div id="tableQueues_paginate class="dataTables_paginate paging_simple_numbers"><ul id="queues-page-tab" class="pagination"></ul></div></div>');
	$("#inbound-queues-detail-container").append(
			'<table class="table" id="queues-detail-table"></table>');
						...
						columns : [
								...
								},
								{
									...
									render : function(data, type, row) {
										...
										return '<button  id="qplay-'
												+ data1
												+ '" type="button" class="btn btn-success btn-xs" onclick="queues.playSound(\''
												+ data1
												+ '\')"><i class="fa fa-play"></i></button>'
												+ '<div class="btn-toolbar btn-toolbar-sound"> '
												+ '<button id="qstop-'
												+ data1
												+ '" class="btn btn-danger btn-xs hidden" onclick="queues.stopSound(\''
												+ data1
												+ '\')" ><i class="fa fa-stop"></i></button>'
												+ '<button id="qdownload-'
												+ data1
												+ '" class="btn btn-success btn-xs hidden" onclick="queues.downloadSound(\''
												+ data
												+ '\')" ><i class="fa fa-download"></i></button>'
												+ '<button id="qsound_error-'
												+ data1
												+ '" class="btn btn-xs btn-danger hidden" ><i class="fa fa-exclamation-triangle"></i></button>'
												+ '</div>';
	...
	for (var k = 0; k < e.tabs.length; k++) {
		...
		var a = $(
				'<li/>',
				{
					...
					'html' : '<a  href="#" aria-controls="queues-detail-table" onclick="queues.queueDetail(\''
							+ e.tabs[k].p
							+ '\',\''
							+ date
							+ '\',\''
	...
	queueDetailTable
			...
			.each(
					function(index) {
						...
						var tr = row.node();
						if ($(row.node().childNodes[3].childNodes[0])
								.attr('id') != "qplay-" + data) {
							$(row.node().childNodes[3].childNodes[0])
									.removeClass("hidden");
							$(
									row.node().childNodes[3].childNodes[1].childNodes[1])
									.addClass("hidden");
							$(
									row.node().childNodes[3].childNodes[1].childNodes[2])
									.addClass("hidden");
							$(
									row.node().childNodes[3].childNodes[1].childNodes[3])
									.addClass("hidden");
						}
...
queues.showQueueSpell = function(page, agentid) {
